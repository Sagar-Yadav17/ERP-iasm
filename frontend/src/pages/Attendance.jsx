      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-x-auto">
        <table className="w-full min-w-[800px] text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {['Employee', 'Department', 'Status', 'Current'].map(
                (h) => (
                  <th
                    key={h}
              ...
              data.map(({ employee, attendance }) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 whitespace-nowrap">
                    <select
                      value={
                        localStatus[employee._id] || 'present'
                      }
                      onChange={(e) =>
                        handleStatusChange(
                          employee._id,
                  ...
                  <td className="px-5 py-4 whitespace-nowrap">
                    {attendance ? (
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${statusColor[attendance.status]}`}
                      >
                        {attendance.status}
                      </span>
                    ) : (
